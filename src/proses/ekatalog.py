...
# Baca dataset E-Katalog Komoditas Detail
dfECAT_KD = read_df_duckdb(datasets['ECAT_KD'])

# Baca dataset E-Katalog Penyedia Detail
dfECAT_IS = read_df_duckdb(datasets['ECAT_IS'])

# Baca dataset E-Katalog Instansi Satker
dfECAT_PD = read_df_duckdb(datasets['ECAT_PD'])

st.dataframe(dfECAT)
st.dataframe(dfECAT_KD)
st.dataframe(dfECAT_IS)
st.dataframe(dfECAT_PD)

dfECAT1 = dfECAT.merge(dfECAT_KD, how='left', on='kd_komoditas')
st.dataframe(dfECAT1)

# dfECAT_OK = (dfECAT
#             .merge(dfECAT_KD, how='left', on='kd_komoditas')
